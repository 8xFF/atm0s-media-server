...
pub async fn run_token_generate_server(http_port: u16, _opts: TokenGenerateArgs, secret: &str, token_signer: Arc<dyn SessionTokenSigner + Send + Sync>) -> Result<(), &'static str> {
    let mut http_server: HttpRpcServer<()> = crate::rpc::http::HttpRpcServer::new(http_port);
