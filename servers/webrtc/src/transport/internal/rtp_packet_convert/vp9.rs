...
pub fn payload_parse(payload: &[u8], rid: Option<u16>) -> (bool, Option<Vp9Svc>) {
    let mut vp9_header = Vp9Header::default();
    if let Ok(res) = vp9_header.parse_from(payload, rid) {
        res
    } else {
        (false, None)
    }
