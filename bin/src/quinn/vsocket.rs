...
pub struct VirtualUdpSocket {
    ...
    tx: Sender<socket::Control>,
    close_socket_tx: UnboundedSender<u16>,
}

impl VirtualUdpSocket {
    pub fn new(node_id: u32, port: u16, tx: Sender<socket::Control>, rx: Receiver<NetworkPkt>, close_socket_tx: UnboundedSender<u16>) -> Self {
        Self {
            port,
        ...
        match transmit.destination {
            SocketAddr::V4(addr) => {
                ...
                if self.tx.capacity() > 0
                    && self
                        .tx
                        .try_send(socket::Control::SendTo(
                            self.port,
                            u32::from_be_bytes(addr.ip().octets()),
                            addr.port(),
                            transmit.contents.to_vec().into(),
                            transmit.ecn.map(|x| x as u8).unwrap_or(0),
                        ))
                        .is_ok()
                {
                    Ok(())
