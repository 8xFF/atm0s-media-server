    ...
    async fn call(&self, req: Request) -> Result<Self::Output, Error> {
        ...
        match E::get(&self.path) {
            Some(content) => {
                ...
                if req.headers().get(header::IF_NONE_MATCH).map(|etag| etag.to_str().unwrap_or("000000").eq(&hash)).unwrap_or(false) {
                ...
                Ok(Response::builder().header(header::CONTENT_TYPE, mime.as_ref()).header(header::ETAG, hash).body(body))
            }
