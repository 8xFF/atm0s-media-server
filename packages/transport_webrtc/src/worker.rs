...
impl<ES: MediaEdgeSecure> MediaWorkerWebrtc<ES> {
    ...
    pub fn spawn(&mut self, app: AppId, remote: IpAddr, session_id: u64, variant: VariantParams<ES>, offer: &str) -> RpcResult<(bool, String, usize)> {
            ...
            GroupInput::Ext(owner, ext) => {
                ...
                if self.endpoints.has_task(owner.index()) {
                    ...
                    match ext {
                        ...
                        ExtIn::RestartIce(req_id, variant, app, remote, useragent, req, extra_data, record) => {
