...
impl ConnectorHookProducer {
    ...
    fn on_peer_event(&mut self, from: NodeId, ts: u64, session: u64, ev: peer_event::Event) -> Option<()> {
        let hook_data: Option<HookEvent> = match ev {
            ...
            peer_event::Event::RemoteTrackStarted(_params) => None,
            peer_event::Event::RemoteTrackEnded(_params) => None,
            peer_event::Event::LocalTrack(_params) => None,
            peer_event::Event::LocalTrackAttach(_params) => None,
            peer_event::Event::LocalTrackDetach(_params) => None,
